...
export const SchedDisplayProvider = ({ children }) => {
  ...
  useEffect(() => {
    if (!authToken) {
      setSchedDoctors([]);
      setSchedDoctorsSched([]);
      setLoading(false);
    } else {
      fetchSchedData();
    }
  }, [authToken, fetchSchedData]);
  ...
  const UpdateSchedDoctor = async (dataId, values) => {
    ...
    try {
      ...
      if (updated?._id) {
        setSchedDoctors((prev) =>
          prev.map((u) =>
            u._id.toString() === updated._id.toString()
              ? { ...u, timeSlots: updated.timeSlots, status: updated.status }
              : u
          )
        );
        setModalStatus("success");
  ...
  const Deletedata = async (id) => {
    try {
      const res = await axiosInstance.delete(
        `${import.meta.env.VITE_REACT_APP_BACKEND_BASEURL}/api/v1/Schedule/${id}`,
        {
          headers: { Authorization: `Bearer ${authToken}` },
        }
      );
