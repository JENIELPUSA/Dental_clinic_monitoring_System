...
const noScheduleEverCron = require("./Utils/noScheduleEverCheck");
...
io.on("connection", (socket) => {
  ...
  socket.on("newRequest", (data) => {
    ...
    for (const linkId in global.connectedUsers) {
      ...
      if (
        (doctor_id &&
          (role === "admin" || role === "staff" || linkId === doctor_id)) ||
        (!doctor_id && (role === "doctor" || role === "staff"))
      ) {
  ...
  // Frontend request to refresh data
...
require("./Utils/DeleteUsingCronJob");
noScheduleEverCron();
